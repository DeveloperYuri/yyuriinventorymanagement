...
@section('content')
    <main id="main" class="main">
        ...
        @if (Auth::user()->is_role == 2 || Auth::user()->is_role == 1)
            <div class="pagetitle">
                <a href="{{ route('asset-out.create') }}" class="btn btn-primary" dusk="createnewassettoolsout">Create New
                    Asset Tools Out</a>
            </div>
        @endif
                                ...
                                @if (Auth::user()->is_role == 2 || Auth::user()->is_role == 1)
                                    <div class="d-flex gap-2">
                                        {{-- <a href="{{ route('export.asset-stock-out') }}" class="btn btn-success"
                                            target="_blank">Print
                                            PDF</a> --}}
                                        <a href="{{ route('export.asset-stock-out', ['start_date' => request('start_date'), 'end_date' => request('end_date')]) }}"
                                            class="btn btn-success">Print PDF</a>

                                        <a href="{{ route('assetstockout.export.excel', [
